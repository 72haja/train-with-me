 ...
 */
export function searchConnections(originId: string, destinationId: string): Connection[] {
    ...
    const filtered = mockConnections.filter(connection => {
        const hasOrigin = connection.stops.some(stop => stop.station.id === originId);
        const hasDestination = connection.stops.some(stop => stop.station.id === destinationId);
        ...
        const originIndex = connection.stops.findIndex(stop => stop.station.id === originId);
