...
- **Friend matching by `trip_id`**, not `connection_id`. Two friends boarding the same physical train at different stations have different `connection_id`s but the same `trip_id`.
